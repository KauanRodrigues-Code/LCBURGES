...
function addToCartFromModal() {
    ...
    cart.push({ ...selectedProduct, cartId: Date.now(), extras: selectedExtras, obs: obs, totalPrice: selectedProduct.price + extrasTotal });
    updateCart(); closeModal(); showToast(`${selectedProduct.name} adicionado!`);
}
...
function removeFromCart(cartId) {
    ...
    updateCart(); showToast("Item removido", "error");
}

function clearCart() {
    if(cart.length === 0) return;
    if(confirm("Deseja limpar toda a sacola?")) {
        cart = []; updateCart(); showToast("Você limpou sua Sacola", "error");
    }
}
        ...
        <div class="cart-item-card">
            <div class="cart-item-info">
                <div>
                    ...
                    ${item.obs ? `<div class="cart-obs">📝 ${item.obs}</div>` : ''}
                </div>
...
function finishOrder() {
    ...
    const sub = cart.reduce((a, b) => a + b.totalPrice, 0);
    const tax = delType === "entrega" ? 5 : 0;
    text += `\n💳 *Pagamento:* ${document.getElementById("payment-method").value}\n💰 *Total: R$ ${(sub + tax).toFixed(2)}*`;
    window.open(`https://wa.me/5543988230563?text=${encodeURIComponent(text)}`);
}
